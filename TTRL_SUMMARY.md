...
- ✅ Reward-mode switch (env sparse default, optional efficiency)
- ✅ Test-time RL mode flag
- ✅ Fallback to standard 0/1 reward when preferred
...
### Reward Modes
```
Default: reward = 1.0 if success else 0.0 (environment sparse)
Optional: reward = 1.0 / (1.0 + steps/max_steps) if success else 0.0 (efficiency)

Switch via config:
- '+test_time_rl_reward=env_sparse'  # default
- '+test_time_rl_reward=efficiency'  # enable shaping
```
...
actor_rollout_ref.model.max_steps: 500  # Needed only if using efficiency reward
```
...
test_time_rl_reward: env_sparse    # Reward mode (env_sparse | efficiency)
```
...
python -m verl.trainer.main_ppo \
  ...
  '+test_time_rl_reward=env_sparse'
```
...
└──────────────┬──────────────────────────────┘
               ...
               ▼
┌──────────────────────────────────────────────┐
│ Compute Rewards (main_ppo.py)               │
│ ├─ Default: 1.0 if complete else 0.0       │
│ └─ Optional: efficiency shaping if enabled │
└──────────────┬──────────────────────────────┘
               │
               ▼
┌──────────────────────────────────────────┐
...
| **Reward signal** | Binary (0/1) | Configurable (binary or efficiency) |
| **Learning signal** | Success/failure | Success + optional speed tradeoffs |
| **Compute cost** | 1× | N× |
