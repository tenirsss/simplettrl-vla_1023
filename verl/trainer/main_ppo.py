...
class RobRewardManager():
    ...
    def __init__(self, num_examine, config) -> None:
        ...
        self.config = config
        # Test-Time RL mode: default to sparse env reward, efficiency reward optional
        self.test_time_rl_mode = config.get("test_time_rl", False)
        # reward mode can be "env_sparse" (default) or "efficiency"
        self.test_time_rl_reward_mode = config.get("test_time_rl_reward", "env_sparse")
        if self.test_time_rl_reward_mode not in {"env_sparse", "efficiency"}:
            raise ValueError(f"Unsupported test_time_rl_reward mode: {self.test_time_rl_reward_mode}")
    ...
    def verify(self, data):
        ...
        score = []
        if self.test_time_rl_mode and self.test_time_rl_reward_mode == "efficiency":
